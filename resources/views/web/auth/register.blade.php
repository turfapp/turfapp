...
@section('form')
    <form id="register" class="ta:form" aria-live="assertive" accept-charset="UTF-8" method="post" action="{{ route('auth.register') }}">
        ...
        <div class="ta:form-group">
            ...
            <input id="account_name_field" type="text" name="name" autocomplete="off" autocapitalize="none" aria-required="true" required="required" spellcheck="false" placeholder="{{ __('Name') }}" autofocus="autofocus" />
        ...
        <div class="ta:form-group">
            ...
            <input id="account_email_field" type="text" name="email" autocomplete="off" autocapitalize="none" aria-required="true" required="required" spellcheck="false" placeholder="{{ __('E-Mail Address') }}" autofocus="autofocus" />
            @error('email')<p class="ta:form-text ta:form-text-error">{{ $message }}</p>@enderror
        </div>
        <div class="ta:form-group">
            <label for="password_field">{{ __('Password') }}</label>
            <input id="password_field" type="password" name="password" aria-required="true" required="required" autocomplete="off" placeholder="{{ __('Password') }}" />
